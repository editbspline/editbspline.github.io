...
import { add, isVector, safeProduct, TupleVector } from './Vector';
...
export class Polynomial implements Evaluable {
  ...
  /**
   * Recalculates the degree of this polynomial based on
   * {@code this.coefList} solely.
   */
  #normalize() {
    let zeroCoefs = 0;
    for (let coefIdx = 0; coefIdx < this.coefList.length &&
            this.coefList[coefIdx] === 0; coefIdx++) {
      ++zeroCoefs;
    }
  ...
  multiply(scale: number | Polynomial | TupleVector) {
    if (typeof scale === 'number' || isVector(scale)) {
