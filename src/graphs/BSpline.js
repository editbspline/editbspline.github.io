...
import { BasicSpline, FlattenerBasicSpline } from './Spline';
...
export type StdBSpline = BasicSpline & {
...
export function StepBSpline(
...
): StdBSpline | Array<SimpleBasisPolynomial> {
  ...
  const basisPolys: Array<SimpleBasisPolynomial> = [];

  for (let knotOff = 0; knotOff < knotVector.length - 1; knotOff++) {
    let basisPoly = new SimpleBasisPolynomial(CONSTANT_ONE,
      new SimpleRange(
  ...
  if (controlPoints) {
    ...
    const stepBasisSpline = new BasicSpline(basisPolys);
...
export function BSpline(
  ...
  let lowerBasisPolys = ((StepBSpline(knotVector): any): Array<SimpleBasisPolynomial>);
  let buildOrder = 2;
  ...
  while (buildOrder <= splineOrder) {
    ...
    for (let i = 0; i < lowerBasisPolys.length - 1; i++) {
      ...
      const higherBasis = new BasicSpline([
        lowerBasisPolys[i]
  ...
  const bspline: any = (doNotFlatten) ?
    // $FlowFixMe: https://github.com/facebook/flow/issues/7763
    new BasicSpline(lowerBasisPolys) :
    // $FlowFixMe: https://github.com/facebook/flow/issues/7763
    FlattenerBasicSpline.flatten(new BasicSpline(lowerBasisPolys));
